...
$$.box = function (p) {
    ...
    var mat = {};
    if (p.texture) mat.map = p.texture; else mat.color = p.color;
...
$$.hasPlayerMapCollision = function (p) {
    if (p.y > $s.map.wallHeight) return false;
    // FIXME: there are problems with wall with added holes (test)

    var half = $s.player.bboxDim;
    // FIXME: we suppose the player is not wider than 1

    var target = $$.vec();
    var xs = [Math.floor(p.x-half), Math.ceil(p.x+half)];
    var zs = [Math.floor(p.z-half), Math.ceil(p.z+half)];

    for (var x = xs[0]-1; x < xs[1]+1; ++x) {
        for (var z = zs[0]-1; z < zs[1]+1; ++z) {
            var sx = new THREE.Line3($$.vec(x, 0, z), $$.vec(x+1, 0, z));
            var sz = new THREE.Line3($$.vec(x, 0, z), $$.vec(x, 0, z+1));

            if ($m.coordToIndex(x, z, 'r') in mMap) {
                sx.closestPointToPoint(p, true, target)
                if (target.sub(p).length() < half) return true;
            }
            if ($m.coordToIndex(x, z, 'd') in mMap) {
                sz.closestPointToPoint(p, true, target)
                if (target.sub(p).length() < half) return true;
            }
        }
    }
